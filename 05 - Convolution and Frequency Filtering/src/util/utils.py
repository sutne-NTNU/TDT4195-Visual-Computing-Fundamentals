...
def test_model_forward_pass(model, images, batch_size):
    """ Test wether the model can perform a singe forward pass """
    output = model(images)
...
def show_and_save_loss_plot(filename: str, ylim: list = [0.0, 0.5]):
    plt.ylim(ylim)
    plt.legend()
    plt.xlabel("Global Training Step")
    plt.ylabel("Cross Entropy Loss")
    plt.savefig(image_output_dir.joinpath(filename))
    plt.show()
...
def create_high_pass_frequency_kernel(im: np.array,
    ...
    rr, cc = skimage.draw.disk(center=(center_row, center_col), radius=radius)
    kernel[rr, cc] = 0.0
...
def fft_to_image(fft):
    fft = np.fft.fftshift(fft)
    return np.log(magnitude(fft) + 1)
