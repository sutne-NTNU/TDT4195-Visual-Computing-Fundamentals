...
impl ShaderBuilder {
    ...
    pub unsafe fn attach_file(
        ...
        if let Some(extension) = path.extension() {
            ...
            let shader_src = std::fs::read_to_string(path).expect(&format!("Failed to read shader source. {}", shader_path));
